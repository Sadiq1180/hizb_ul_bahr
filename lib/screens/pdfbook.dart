...
//   // Plays the page-turn sound effect. A small pool of players is used so
//   // rapid successive flips each get their own player instead of cutting
//   // each other off.
//   final _FlipSoundPlayer _flipSound = _FlipSoundPlayer();

//   // The very first "page change" the curl view reports is the programmatic
//   // jump to the saved reading position on open, not a real user flip. This
//   // suppresses the sound for exactly that one event.
...
//               // The jump below will fire onPageChanged just like a real
//               // flip would; make sure that one event stays silent.
...
//                                 edgeZoneWidth: 0.30,
//                                 onPageChanged: (page) {
//                                   // Grab + clear the suppression flag first,
//                                   // before anything else can early-return.
...
// /// Plays the page-turn sound effect using a small round-robin pool of
// /// [AudioPlayer]s, so that flipping several pages in quick succession gets
// /// an audible "flip" for each one instead of the sound being cut off by
// /// the next flip restarting the same player.
// ///
// /// Uses [PlayerMode.lowLatency], which on Android/iOS uses the platform's
// /// low-latency sound-pool style playback path — the right mode for short,
// /// frequently-repeated UI sound effects like this one (as opposed to the
// /// default mode, which is tuned for longer media playback).
// class _FlipSoundPlayer {
//   _FlipSoundPlayer({
//     this.poolSize = 3,
//     this.assetPath = 'sounds/page_flip.mp3',
//     this.volume = 0.6,
//   }) : _players = List.generate(poolSize, (_) {
...
//   /// Path relative to the `assets/` folder, e.g. `assets/sounds/page_flip.mp3`
//   /// declared as `sounds/page_flip.mp3` here (AudioPlayer's AssetSource is
//   /// relative to `assets/` by convention on most platforms — adjust if your
//   /// pubspec.yaml declares assets under a different root).
//   final String assetPath;
//   final double volume;
...
//     try {
//       // Stop + replay rather than seek, so a flip that lands on a player
//       // still finishing the previous flip cuts over cleanly.
//       await player.stop();
//       await player.play(AssetSource(assetPath));
//     } catch (_) {
//       // A missed sound effect is not worth surfacing to the user or
//       // breaking the reading experience over.
//     }
//   }
